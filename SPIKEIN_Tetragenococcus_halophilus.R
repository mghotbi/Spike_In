...
# Create new phyloseq object with scaled ASV counts
...
# Calculate scaling factors for Tetragenococcus_halophilus and save results
...
physeq16S_adj_scaled <- round(otu_table(physeq16S_adj_scaled) * scaling_factor_Tetragenococcus_halophilus)
write.csv(physeq16S_adj_scaled, file = "physeq16S_adj_scaled_Absolute.csv")

# Create new phyloseq object with absolute abundance
otu <- read.csv("physeq16S_adj_scaled_Absolute.csv", header = TRUE, sep = ",", row.names = 1)
...
adj_scaled_spiked_16S <- phyloseq(otu_table(otumat, taxa_are_rows = TRUE),
                           tax_table(adj_spiked_16S),
                           phy_tree(random_tree),
                           sample_data(adj_spiked_16S))                     